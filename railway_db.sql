-- Base de datos para Railway - Sistema de Consejería y Tutoría
-- NO incluye CREATE DATABASE porque Railway ya tiene la BD "railway"
-- Fecha: 3 de diciembre de 2025

-- Eliminar tablas si existen (en orden inverso por foreign keys)
DROP TABLE IF EXISTS atenciones;
DROP TABLE IF EXISTS usuarios;
DROP TABLE IF EXISTS temas_consejeria;
DROP TABLE IF EXISTS estudiantes;
DROP TABLE IF EXISTS docentes;
DROP TABLE IF EXISTS semestres;
DROP VIEW IF EXISTS vista_atenciones_resumen;
DROP VIEW IF EXISTS vista_atenciones_detalle;
...
-- Tabla de Usuarios del Sistema (PRIMERO, antes de atenciones)
...
INSERT INTO usuarios (username, password, nombre_completo, rol, docente_id) VALUES
('admin', 'admin123', 'Administrador del Sistema', 'admin', NULL),
('docente1', 'docente123', 'María González Pérez', 'docente', 1),
('coordinador', 'coord123', 'Carlos Rodríguez López', 'coordinador', 2);
...
-- Verificación final
SELECT 'Base de datos Railway configurada correctamente' AS mensaje;
SELECT COUNT(*) AS total_usuarios FROM usuarios;
SELECT COUNT(*) AS total_docentes FROM docentes;
SELECT COUNT(*) AS total_estudiantes FROM estudiantes;
SELECT COUNT(*) AS total_temas FROM temas_consejeria;
SELECT COUNT(*) AS total_semestres FROM semestres;
